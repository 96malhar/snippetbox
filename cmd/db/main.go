...
const superUserDSN = "postgresql://postgres:postgres@localhost:5432/?sslmode=disable"
...
func teardown() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "teardown",
		Short: "Tears down the database and the database user used by the Snippetbox app",
		Run: func(cmd *cobra.Command, args []string) {
			db, err := sql.Open("postgres", superUserDSN)
			must(err)
